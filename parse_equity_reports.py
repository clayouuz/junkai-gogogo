...
提取信息时需遵循以下规则：
0. **数据格式**：请确保返回的JSON数据格式正确，pdf文档中的每一个交易数据都要提取出独立的一行，禁止合并。
...
3. **变动方向**：从"增持"和"减持"和"不变"中选择文档中所涉及的变动方向，你需要根据文档内容充分判断是增持还是减持，以下两种情况视为不变：第一，股票在同一实际控制人下属不同直接持股股东之间的划转；第二，一致行动人关系的解除。如果无法判断请留空。
